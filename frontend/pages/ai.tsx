  ...
  return (
    <>
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Ask questions about NHS waiting list data. Powered by Claude.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div
          className="md:col-span-2 flex flex-col bg-slate-900 border border-slate-800 rounded-xl overflow-hidden"
          style={{ height: 560 }}
        >
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                ...
                <div>
                  <p className="text-slate-300 font-semibold">
                    Ask anything about NHS waiting lists
                  </p>
                </div>
              </div>
            ) : null}
            ...
            {messages.map((m, i) => (
              ...
              >
                {m.role === 'assistant' ? (
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-1 ${
                      m.agentic ? 'bg-slate-800' : 'bg-nhs-navy'
                    }`}
                  >
                    AI
                  </div>
                ...
                >
                  ...
                  {m.role === 'assistant' && !m.streaming && m.content.length > 50 && (
                    <div className="mt-3 p-3 bg-slate-700/50 border border-slate-600 rounded-lg">
                      <div className="text-xs font-bold text-slate-300 mb-1">Recommended Actions:</div>
                      <ul className="list-disc pl-4 text-xs text-slate-400 space-y-1">
                        <li>Consider localized capacity increases focusing on highest deprivation quartiles.</li>
                        <li>Review specialty bottlenecks for potential independent sector utilization.</li>
                      </ul>
                    </div>
                  )}
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      {!m.agentic && (
                        <span className="text-[10px] bg-slate-700 text-slate-400 px-2 py-0.5 rounded border border-slate-600">
                          Data logic: {region ? `IMD + RTT rates for ${region}` : 'National averages'}
                        </span>
                      )}
                    </div>
                  ) : null}
                ...
                {m.role === 'user' ? (
                  <div className="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 text-xs font-bold flex-shrink-0 mt-1">
                    You
                  </div>
                ) : null}
              </div>
          ...
          <form onSubmit={handleSubmit} className="border-t border-slate-800 p-3 flex gap-2">
            <input
              ...
              placeholder={deepMode ? 'Ask a deep data question...' : 'Ask about waiting times, inequality, regions...'}
