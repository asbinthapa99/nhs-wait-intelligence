...
const sidebarIcons = [
  ...
  { icon: Users, href: '/mp', label: 'MP Lookup' },
  { icon: Clock, href: '/estimate', label: 'Wait Estimator' },
  { icon: Building2, href: '/trusts', label: 'Trust League' },
  { icon: Code2, href: '/api-docs', label: 'API Docs' },
  { icon: Banknote, href: '/budget', label: 'Budget vs Outcomes' },
  { icon: UserMinus, href: '/vacancies', label: 'Vacancies' },
  { icon: CalendarClock, href: '/projection', label: '92% Projection' },
  { icon: Globe, href: '/nations', label: 'UK Nations' },
  { icon: Wallet, href: '/private-cost', label: 'Private Cost' },
  { icon: ScrollText, href: '/hansard', label: 'Hansard' },
  { icon: HelpCircle, href: '/guide', label: 'User Guide' },
    ...
    <div className="min-h-screen bg-[#f5f5f5] text-[#1a1a1a] flex flex-col md:flex-row overflow-hidden selection:bg-blue-100 font-sans">
      ...
      {moreOpen && (
        <>
          ...
          />
          {/* Sheet */}
          <div className="md:hidden fixed bottom-0 left-0 right-0 z-[70] bg-white border-t border-[#e5e5e5] rounded-t-2xl shadow-2xl max-h-[82vh] flex flex-col">
            {/* Handle */}
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-[#f0f0f0] shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-8 h-1 bg-[#e5e5e5] rounded-full mx-auto absolute left-1/2 -translate-x-1/2 top-2" />
                <span className="text-sm font-bold text-[#111]">All Tools</span>
              </div>
              <button onClick={() => setMoreOpen(false)} className="p-1.5 rounded-lg bg-[#f5f5f5] text-[#888] hover:text-[#111] transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-4 py-4 space-y-5 pb-safe">
              {moreGroups.map((group) => (
                <div key={group.label}>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[#bbb] mb-2 px-1">{group.label}</p>
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        <Link
                          key={item.href}
                          href={item.href}
                          className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all active:scale-[0.98] ${active ? 'bg-emerald-50 border border-emerald-100' : 'bg-[#fafafa] border border-[#f0f0f0] hover:border-[#ddd]'}`}
                        >
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${active ? 'bg-white text-emerald-600' : 'bg-white text-[#bbb]'}`}>
                            <Icon size={17} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-bold ${active ? 'text-emerald-700' : 'text-[#666]'}`}>{item.label}</p>
                            <p className="text-[11px] text-[#bbb] truncate">{item.desc}</p>
                          </div>
              ...
              <div className="flex gap-3 pt-2 pb-2">
                {[{ href: '/legal', label: 'Legal' }, { href: '/cookies', label: 'Cookies' }].map(l => (
                  <Link key={l.href} href={l.href} className="text-xs text-[#bbb] hover:text-[#111] transition-colors font-semibold uppercase tracking-wider">{l.label}</Link>
                ))}
              </div>
            </div>
          </div>
        ...
        <header className="hidden md:flex items-center h-16 bg-white border-b border-[#e5e5e5] px-8 shrink-0 relative z-10">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Link href="https://github.com/asbinthapa99/nhs-wait-intelligence" target="_blank" rel="noopener noreferrer"
              className="text-[#666] hover:text-[#666] transition-colors p-1.5">
